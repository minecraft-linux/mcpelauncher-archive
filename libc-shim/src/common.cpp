...
#define DEFINE_OPTIONAL_FUNCTION(funcName) extern void funcName();\
template<class T=void(*)(),class=void> struct has_ ## funcName : std::false_type {};\
template<class T, class... arg> struct has_ ## funcName<T(*)(arg...),std::void_t<decltype(funcName(std::declval<arg>()...))>> : std::true_type {};
DEFINE_OPTIONAL_FUNCTION(strtoul_l);
DEFINE_OPTIONAL_FUNCTION(strtoull_l);
DEFINE_OPTIONAL_FUNCTION(strtol_l);
DEFINE_OPTIONAL_FUNCTION(strtoll_l);
DEFINE_OPTIONAL_FUNCTION(strtof_l);
DEFINE_OPTIONAL_FUNCTION(strtod_l);
DEFINE_OPTIONAL_FUNCTION(strtold_l);
DEFINE_OPTIONAL_FUNCTION(strptime_l);
DEFINE_OPTIONAL_FUNCTION(strftime_l);
#undef DEFINE_OPTIONAL_FUNCTION

template<class T> struct CLocaleExtWrapper : CLocaleExtWrapper<typename remove_all_extends<T>::PointerType> {};
template<class R, class ... arg > struct CLocaleExtWrapper<R (*)(arg...)> {
    template<R (*org)(arg...)> static R rewrite(arg...a, locale_t l) {
        (void)l;
        return org(a...);
    }
};

#define CLocaleExt(name) ([]() { if constexpr(has_ ## name ## _l<decltype(&CLocaleExtWrapper<decltype(&name)>::rewrite<&name>)>::value) { return (decltype(&CLocaleExtWrapper<decltype(&name)>::rewrite<&name>))&name ## _l; } else { return CLocaleExtWrapper<decltype(&name)>::rewrite<&name>; } })()
...
void shim::add_stdlib_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        {"strtoul_l", WithErrnoUpdate(CLocaleExt(strtoul))},
        //{"strtoq", WithErrnoUpdate(strtoq)},
        //{"strtouq", WithErrnoUpdate(strtouq)},
        {"strtoll", WithErrnoUpdate(strtoll)},
        {"strtoll_l", WithErrnoUpdate(CLocaleExt(strtoll))},
        {"strtoull", WithErrnoUpdate(strtoull)},
        {"strtoull_l", WithErrnoUpdate(CLocaleExt(strtoull))},
        {"strtof_l", ARMHFREWRITE(WithErrnoUpdate(CLocaleExt(strtof)))},
        {"strtold_l", ARMHFREWRITE(WithErrnoUpdate(CLocaleExt(strtold)))},
...
void shim::add_time_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        {"strftime", ::strftime},
        {"strptime", ::strptime},
        {"strftime_l", CLocaleExt(strftime)},
        {"strptime_l", CLocaleExt(strptime)},
        {"gmtime", ::gmtime},
        {"gmtime_r", ::gmtime_r},
        {"localtime", ::localtime},
...
void shim::add_string_shimmed_symbols(std::vector<shim::shimmed_symbol> &list) {
    list.insert(list.end(), {
        ...
        {"bzero", ::bzero},
        {"ffs", ::ffs},
        // {"index", Optional(index, (char *(*) (const char *, int))([](const char *str, int ch) -> char* { return (char*)strchr(str, ch); } ))},
        {"index", (char *(*) (char *, int))::index},
        {"rindex", (char *(*) (char *, int))::rindex},
