...
void JniSupport::startGame(ANativeActivity_createFunc *activityOnCreate) {
    ...
    memset(&nativeActivityCallbacks, 0, sizeof(nativeActivityCallbacks));
    memset(&nativeActivity, 0, sizeof(nativeActivity));
    nativeActivity.callbacks = &nativeActivityCallbacks;
    nativeActivity.vm = ((jnivm::VM*)vm.functions->reserved0)->GetJavaVM();
    nativeActivity.assetManager = (AAssetManager *) assetManager.get();
    nativeActivity.env = ((jnivm::VM*)vm.functions->reserved0)->GetJNIEnv();
